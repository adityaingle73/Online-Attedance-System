...
class Attendance:
    def __init__(self):   
        ...
        #----------database connection-----------#
        try:
            self.connection = mysql.connector.connect(host="localhost",password="shrikantvh@26",username="root",database="pythonproject")
        except:
    ...
    def login_sucess(self,username):
        ...
        #------select divisions-----#
        select_division=tk.Label(login_sucess_interface,text="Select Branch")
        select_division.place(x=200,y=50)
        #------divisions combobox-------#
        divisions_combobox=ttk.Combobox(login_sucess_interface,state="readonly",width=30)
        divisions_combobox['values']=("CS","CS2","ENTC","MECH","CIVIL")
        ...
        def getdetails():
            ...
            query = f"SELECT MIS, name FROM {division_name};"
            cursor.execute(query)
            self.data = cursor.fetchall()
            start = 180
            self.checkboxes = []  # create a list to store the checkboxes
            for MIS, name in self.data:
                records_label = ttk.Label(login_sucess_interface, text=f"{MIS} {name}")
            ...
            try:
                ...
                for details in details_with_status:
                    if False in details:
                        ...
                        try:
                            MIS=details[0][0]
                            query = f"UPDATE {division} SET status='Absent',date_day='{selected_date}' where MIS='{MIS}'"
                            cursor=self.connection.cursor()
                            cursor.execute(query)
                            self.connection.commit()
                        except:
                        ...
                        try:
                            MIS=details[0][0]
                            query = f"UPDATE {division} SET status='Present',date_day='{selected_date}' where MIS='{MIS}'"
                            cursor=self.connection.cursor()
                            cursor.execute(query)
                            self.connection.commit()
                        except:
        ...
        def add_student():
            ...
            #------MIS label and entry----#
            MIS_var=tk.StringVar()
            MIS_label=tk.Label(add_student_window,text="MIS")
            MIS_label.place(x=130,y=30)
            MIS_entry=ttk.Entry(add_student_window,textvariable=MIS_var)
            MIS_entry.place(x=100,y=50)
            #------name entry------#
            ...
            select_division_label_combo=tk.Label(add_student_window,text="Select Branch")
            ...
            select_division_combobox['values']=('CS','CS2','ENTC','MECH','CIVIL')
            ...
            def add_new_student():
                ...
                if len(MIS)==0 or len(name)==0 or len(division)==0:
                    messagebox.showwarning("No Empty Fields","Please Fill the details")
                else:
                    try:
                        cursor=self.connection.cursor()
                        query=f"INSERT INTO {division} (MIS,name) values ('{MIS}','{name}')"
                        cursor.execute(query)
                        self.connection.commit()
        ...
        def export_report():
            cursor = self.connection.cursor()
            division = divisions_combobox.get()
            query = f"SELECT MIS, name, status FROM {division}"
            cursor.execute(query)
            data = cursor.fetchall()
            file_path = f"{division}_{date_entry.get_date()}.csv"
            with open(file_path, "w", newline="") as file:
                writer = csv.writer(file)
                writer.writerow(["MIS", "Name", "Status"])
                writer.writerows(data)
            messagebox.showinfo("Export Successful", f"The attendance report has been exported to {file_path}")

        # Create the "Export Report" button
        export_report_button = ttk.Button(login_sucess_interface, text="Export Report", command=export_report)
        export_report_button.place(x=250, y=400)
        ...
        update_attendance.place(x=130,y=400)
        #-----add student----#
